...
glm::vec3 Camera::getPosition() const {
    // Spherical → Cartesian (Y-up right-handed)
...
glm::mat4 Camera::getView() const {
    ...
    // Keep up vector well-defined near the poles
...
glm::mat4 Camera::getProjection(float aspectRatio) const {
    ...
    // Orthographic: half-extents scale with radius so zooming feels natural
...
void Camera::pan(float dx, float dy) {
    ...
    // Scale pan speed proportionally so it feels the same at all zoom levels
